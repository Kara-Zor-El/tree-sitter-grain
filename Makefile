LANGUAGE_NAME := tree-sitter-grain
HOMEPAGE_URL := https://github.com/tree-sitter/tree-sitter-grain
VERSION := 0.1.0
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/grain '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/grain
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/grain
...
# OCaml: needs opam deps (`opam install . --deps-only`) and libtree-sitter on PKG_CONFIG_PATH
# (e.g. `export PKG_CONFIG_PATH="$(brew --prefix tree-sitter)/lib/pkgconfig:$$PKG_CONFIG_PATH"`).
OPAM ?= opam

.PHONY: build-ocaml
build-ocaml:
	$(OPAM) exec -- dune build -p tree-sitter-grain
