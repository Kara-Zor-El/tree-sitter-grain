...
const hasTreeSitterParsingErrors = (source) => parser.parse(source).rootNode.hasError;
...
await runGit(["clone", GRAIN_REPO_URL]).catch(e => {
  ...
  error("Please make sure you have access to the repo and that the URL is correct.");
...
await runGit(["-C", "grain", "checkout", values.commit]).catch(e => {
  ...
  error("If you are trying to test against the latest preview use `--commit=main`.");
  error("If you are trying to test against a specific version use `--commit=grain-vX.Y.Z`.");
  error("If you are trying to test against a specific commit use `--commit=COMMIT_HASH`.");
  error("`--debug` can be used to get more information about the error.");
  if (values.debug) throw new Error(`Failed to clone grain repo: ${e.message}`);
  process.exit(1);
});
...
test("Stdlib", async (t) => {
  for await (const file of getGrainFiles("./grain/stdlib")) {
    await t.test(`Testing \`${file}\``, async () => {
      ...
      assert.ok(!hasTreeSitterParsingErrors(source), `Tree-sitter failed to parse \`${file}\`, for more information investigate the error using the tree-sitter CLI.`);
    });
  }
})